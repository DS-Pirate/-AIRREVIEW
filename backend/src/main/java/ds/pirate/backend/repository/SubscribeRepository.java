...
//    Optional<List<getMySubInfo>> getPostFollwerFollwingInSubByUserid(Long userid, Pageable pageable);
    ...
    @Query(value = "SELECT ss.target_id as targetId " +
            "FROM air_user u " +
            "LEFT JOIN subscrib_list ss ON u.userid = ss.userid_userid " +
            "LEFT JOIN (SELECT u.air_name, u.userid FROM air_user u  ) uu ON ss.target_id = uu.userid " +
            "WHERE u.userid = :userid " +
            "GROUP BY ss.target_id " +
            "ORDER BY follower DESC;", nativeQuery = true)
    Page<subscribList> getPostFollwerFollwingInSubByUseridPage(Long userid, Pageable pageable);
